...
const bonusVideoModules = import.meta.glob('/public/bonus-videos/*.mp4', { as: 'url' });
const bonusThumbJpgModules = import.meta.glob('/public/bonus-videos/*.jpg', { as: 'url' });
const bonusThumbPngModules = import.meta.glob('/public/bonus-videos/*.png', { as: 'url' });
...
const BONUS_VIDEO_FALLBACKS = [
  {
    name: 'Drums',
    url: '/bonus-videos/Drums.mp4',
    thumbnailUrl: '/bonus-videos/Drums.png',
  },
  {
    name: 'IceSkating',
    url: '/bonus-videos/IceSkating.mp4',
    thumbnailUrl: '/bonus-videos/IceSkating.png',
  },
];
...
const useMathGame = () => {
  ...
  const [gameModeType, setGameModeType] = useState(null); // 'lightning' | 'surf' | 'rocket' | 'bonus'
  ...
  const [bonusResumeAfterVideo, setBonusResumeAfterVideo] = useState(false);
  ...
  const [bonusVideoList, setBonusVideoList] = useState([]);
...
const showAnswerSymbolFor300ms = useCallback((payload) => {
  ...
  const normalizeBonusDisplayStreak = useCallback((rawStreak, rawInterval) => {
    const safeInterval =
      Number.isFinite(rawInterval) && rawInterval > 0
        ? rawInterval
        : DEFAULT_BONUS_VIDEO_INTERVAL_CORRECT;
    const safeStreak = Number.isFinite(rawStreak) ? Math.max(0, rawStreak) : 0;
    if (safeStreak === 0) return 0;
    const remainder = safeStreak % safeInterval;
  ...
  const applyBonusState = useCallback((payload) => {
    ...
    const resolvedBonusVideoIntervalCorrect =
      ...
      nextBonusVideoIntervalCorrect > 0
        ? nextBonusVideoIntervalCorrect
    ...
    if (
      ...
      setBonusVideoIntervalCorrect(nextBonusVideoIntervalCorrect);
    }
    if (typeof nextBonusCorrectStreak === 'number') {
  ...
  const clearStoredGameModeContext = useCallback(() => {
    GAME_MODE_STORAGE_KEYS.forEach((key) => localStorage.removeItem(key));
  }, []);
  ...
  useEffect(() => {
    ...
    const loadVideosAndThumbs = async () => {
      const [lightningVideos, surfVideos, rocketVideos, bonusVideos] = await Promise.all([
        ...
        loadVideoSet(bonusVideoModules, bonusThumbJpgModules, bonusThumbPngModules),
      ]);
      ...
      setBonusVideoList(
        bonusVideos && bonusVideos.length > 0 ? bonusVideos : BONUS_VIDEO_FALLBACKS
      );
      ...
      console.log('[GameMode] Detected bonus videos:', bonusVideos);
    };
  ...
  const startOrResumeGameModeRun = useCallback(
    async ({
      ...
      try {
        ...
        if (started?.practice) {
          setInterventionQuestion(mapQuestionToFrontend(started.practice));
          setShowLearningModule(true);
          setIsGameModePractice(true);
          setGameModeInterventionIndex(
            typeof started?.currentIndex === 'number' ? started.currentIndex : 0
          );
  ...
  const startBonusNextQuiz = useCallback(
    async ({ navigateToGameMode = true } = {}) => {
      ...
      try {
        ...
        if (!backendQuestions || backendQuestions.length === 0) {
          throw new Error('No questions returned from quizStart for Bonus Mode.');
        }
        ...
        const fallbackRoute = getRecoveryRoute();
        logClientError('[BonusMode] Failed to start next quiz', e, {
          quizRunId,
          gameModeType: 'bonus',
        });
        setIsGameMode(false);
  ...
  const handleAnswer = useCallback(
    async (selectedAnswer) => {
      ...
      const shouldHoldBonusOptimisticAdvance =
        ...
        optimisticBonusStreak >= resolvedBonusVideoIntervalCorrect;
      const optimisticBonusNextIndex =
        isGameMode &&
        gameModeType === 'bonus' &&
        isCorrect &&
        Array.isArray(quizQuestions) &&
        currentQuestionIndex + 1 < quizQuestions.length &&
        !shouldHoldBonusOptimisticAdvance
          ? currentQuestionIndex + 1
          : null;
      const optimisticSurfNextIndex =
        isGameMode &&
        gameModeType === 'surf' &&
        isCorrect &&
        Array.isArray(quizQuestions) &&
        ...
        (Number.isFinite(questionsPerQuiz) ? questionsPerQuiz : 10) >
          ...
          ? currentQuestionIndex + 1
          : null;
      const optimisticRocketNextIndex =
        isGameMode &&
        gameModeType === 'rocket' &&
        isCorrect &&
        Array.isArray(quizQuestions) &&
      ...
      const optimisticRocketStreak =
        ...
        isCorrect
          ...
          : null;
      const isLightningModeAnswer =
        isGameMode &&
        (gameModeType === 'lightning' ||
          gameModeType == null ||
          (gameModeType !== 'surf' && gameModeType !== 'rocket' && gameModeType !== 'bonus'));
      ...
      const optimisticLightningTotal =
        localLightningFastAnswer
          ...
          : Number.isFinite(lightningCount)
            ? lightningCount
            : 0;
      const shouldHoldLightningOptimisticAdvance =
        localLightningFastAnswer &&
        (optimisticLightningTotal >= lightningTargetCorrect ||
          optimisticLightningTotal % 5 === 0);
      const optimisticLightningNextIndex =
        isLightningModeAnswer &&
        isCorrect &&
        Array.isArray(quizQuestions) &&
        currentQuestionIndex + 1 < quizQuestions.length &&
        !shouldHoldLightningOptimisticAdvance
          ? currentQuestionIndex + 1
          : null;
      const optimisticLightningQuizStreak =
        isLightningModeAnswer ? (isCorrect ? currentQuizStreak + 1 : 0) : null;
      const playedImmediateGameModeFeedback =
        isGameMode &&
        (gameModeType === 'bonus' ||
          gameModeType === 'surf' ||
          gameModeType === 'rocket' ||
  ...
  try {
    ...
    if (optimisticBonusNextIndex != null) {
      // Bonus mode can feel sluggish if we wait for the backend before swapping the question.
      // Advance the UI immediately, but keep the submission lock active until the API resolves.
      setCurrentQuestionIndex(optimisticBonusNextIndex);
      setCurrentQuestion(quizQuestions[optimisticBonusNextIndex]);
      questionStartTimestamp.current = Date.now();
    }
    ...
    if (optimisticRocketNextIndex != null) {
      setCurrentQuestionIndex(optimisticRocketNextIndex);
      setCurrentQuestion(quizQuestions[optimisticRocketNextIndex]);
      questionStartTimestamp.current = Date.now();
    }
    if (optimisticLightningNextIndex != null) {
      setCurrentQuestionIndex(optimisticLightningNextIndex);
      setCurrentQuestion(quizQuestions[optimisticLightningNextIndex]);
      questionStartTimestamp.current = Date.now();
    }
    if (optimisticSurfStreak != null) {
      setSurfCorrectStreak(optimisticSurfStreak);
    }
    if (optimisticRocketStreak != null) {
      setRocketCorrectStreak(optimisticRocketStreak);
    }
    if (isLightningModeAnswer) {
      if (localLightningFastAnswer) {
        setLightningCount(optimisticLightningTotal);
      }
      if (optimisticLightningQuizStreak != null) {
        setCurrentQuizStreak(optimisticLightningQuizStreak);
      }
      setTransientStreakMessage(null);
    }

      if (playedImmediateGameModeFeedback) {
        if (isLightningModeAnswer) {
          if (!isCorrect) audioManager.playWrongSound();
          else if (localLightningFastAnswer) audioManager.playLightningSound();
    ...
    const playGameModeAnswerFeedback = () => {
      if (playedImmediateGameModeFeedback) return;
    ...
    if (out?.practice) {
      ...
      if (isGameMode && practiceMode === 'bonus') {
        setRocketPracticeFact(null);
        setRocketPracticeReverse(null);
        navigate('/learning', { replace: true });
        return;
    ...
    if (resolvedGameModeType === 'rocket') {
      ...
      const rocketCompleted =
        !!out?.completed &&
        (out?.bonusRequired || out?.rocketComplete || out?.beltAwarded || out?.passed || out?.levelAwarded);
    ...
    if (resolvedGameModeType === 'bonus') {
      ...
      const bonusCompleted =
        !!out?.completed && (out?.bonusComplete || out?.beltAwarded || out?.passed || out?.forcePass);
      if (bonusCompleted) {
        playGameModeAnswerFeedback();
        applyBonusState(out);
        setIsTimerPaused(true);
      ...
      if (!quizQuestions || quizQuestions.length === 0 || nextIndex >= quizQuestions.length) {
        await startBonusNextQuiz({ navigateToGameMode: false });
        setIsAnimating(false);
        return;
      }
    ...
    if (optimisticLightningNextIndex == null || nextIndex !== optimisticLightningNextIndex) {
      setCurrentQuestionIndex(nextIndex);
      setCurrentQuestion(quizQuestions[nextIndex]);
    }
  ...
  const handlePracticeAnswer = useCallback(
    async (questionId, answer) => {
      ...
      try {
        ...
        if (out?.bonusQuizRestarted) {
          if (out?.next) {
            const nextQ = mapQuestionToFrontend(out.next);
            setCurrentQuestion(nextQ);
          }
          if (typeof out?.nextIndex === 'number') {
            setCurrentQuestionIndex(Math.max(0, out.nextIndex));
          }
